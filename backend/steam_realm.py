"""Steam-related utilities used across SteamLord backend modules."""
...
def detect_steam_install_path() -> str:
    ...
    logger.log(f"SteamLord: Steam install path set to {_STEAM_INSTALL_PATH}")
...
def _find_steam_path() -> str:
    ...
    if sys.platform.startswith("win") and winreg:
        try:
            ...
            logger.warn(f"SteamLord: Failed to read Steam path from registry: {exc}")
...
def has_lua_for_app(appid: int) -> bool:
    try:
        ...
        logger.error(f"SteamLord (steam_realm): Error checking Lua scripts for app {appid}: {exc}")
        return False
...
def get_game_install_path_response(appid: int) -> Dict[str, any]:
    ...
    if not os.path.exists(library_vdf_path):
        logger.warn(f"SteamLord: libraryfolders.vdf not found at {library_vdf_path}")
    ...
    try:
        ...
        logger.warn(f"SteamLord: Failed to parse libraryfolders.vdf: {exc}")
    ...
    if not library_path:
        logger.log(
            f"SteamLord: appid {appid} not in libraryfolders.vdf, searching all libraries for appmanifest"
        )
        for lib_path in all_library_paths:
            ...
            if os.path.exists(candidate_path):
                ...
                logger.log(f"SteamLord: Found appmanifest at {appmanifest_path}")
                break
    else:
    ...
    if not library_path or not appmanifest_path or not os.path.exists(appmanifest_path):
        logger.log(f"SteamLord: appmanifest not found for {appid} in any library")
    ...
    try:
        ...
        logger.warn(f"SteamLord: Failed to parse appmanifest: {exc}")
        return {"success": False, "error": "Failed to parse appmanifest"}

    app_state = manifest_data.get("AppState", {})
    install_dir = app_state.get("installdir", "")
    if not install_dir:
        logger.warn(f"SteamLord: installdir not found in appmanifest for {appid}")
    ...
    if not os.path.exists(full_install_path):
        logger.warn(f"SteamLord: Game install path does not exist: {full_install_path}")
        return {"success": False, "error": "Game directory not found"}

    logger.log(f"SteamLord: Game install path for {appid}: {full_install_path}")
    return {
        "success": True,
...
def open_game_folder(path: str) -> bool:
    ...
    try:
        ...
        logger.warn(f"SteamLord: Failed to open game folder: {exc}")
        return False
...
__all__ = [
    ...
    "open_game_folder",
    "open_game_folder",
    "clear_steam_browser_cache",
]


def clear_steam_browser_cache() -> Tuple[bool, str]:
    """
    Clears Steam's web browser and overlay cache to fix UI issues.
    Targets:
      - config/htmlcache
    ...
    steam_path = _find_steam_path()
    if not steam_path:
        return False, "Steam path not found"

    targets = [
        os.path.join(steam_path, "config", "htmlcache"),
        os.path.join(steam_path, "config", "overlayhtmlcache"),
        os.path.join(steam_path, "appcache", "httpcache"),
    ]
    ...
    for target in targets:
        if os.path.exists(target):
            try:
                ...
                success_count += 1
                logger.log(f"SteamLord: Cleared cache at {target}")
            except Exception as exc:
                errors.append(f"Failed to clear {os.path.basename(target)}: {exc}")
                logger.warn(f"SteamLord: Failed to clear cache {target}: {exc}")
        else:
            # If it doesn't exist, count as success (already clean)
            success_count += 1

    if errors:
        return False, "; ".join(errors)
    
    return True, "Cache cleared. Please restart Steam."
